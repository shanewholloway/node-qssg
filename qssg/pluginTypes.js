...
var BasicPlugin, CombinedPluginBase, CommonPluginBase, CompileRenderPlugin, CompiledPlugin, DirPluginBase, FilePluginBase, ModulePlugin, PipelinePlugin, RenderedPlugin, StaticPlugin, exports, makeRefError, pluginTypes, qpluginKinds, splitExt,
...
CommonPluginBase = (function() {
  ...
  CommonPluginBase.prototype.pluginProtocol = ['adapt', 'rename', 'bindRender', 'bindContext', 'render', 'context'];
...
DirPluginBase = (function(_super) {
  ...
  DirPluginBase.prototype.pluginProtocol = CommonPluginBase.prototype.pluginProtocol.concat(['contentDir', 'bindContextDir']);
  ...
  DirPluginBase.prototype.contentDir = function(entry, callback) {
    ...
    if (entry.ext.length === 0) {
      ...
      this.bindRender(entry, callback);
    }
    return entry.walk();
  };

  DirPluginBase.prototype.bindContextDir = function(entry, callback) {
    var ctree;
    if (entry.ext.length > 0) {
  ...
  DirPluginBase.prototype.render = function(entry, vars, answerFn) {
    var di, _ref;
    di = (_ref = entry.contentTree) != null ? _ref.items[entry.name] : void 0;
    if ((di == null) || (di.renderFn == null)) {
      return answerFn("Entry '" + entry.name + "' not defined for composite");
    } else {
      return di.renderFn.apply(di, arguments);
    }
  };
